...
- Current store: `useAuthStore` — `session`, `user`, `isAuthenticated`, `isInitialized`, `setSession(session)`, `signUp(email, password, name)`, `signIn(email, password)`, `signOut()`, `resetPassword(email)`, `updateProfile({ name?, avatarUrl? })`, `uploadAvatar(localUri)`.
- `updateProfile`/`uploadAvatar` follow the same rule: they never call `set()` directly — `updateProfile` calls `supabase.auth.updateUser({ data })`, whose result flows back through the `USER_UPDATED` event on the `onAuthStateChange` listener into `setSession`. `uploadAvatar` only uploads to Supabase Storage and returns the public URL; callers must still call `updateProfile({ avatarUrl })` to persist it.
- Stores that don't hold Supabase-derived state (e.g. `useThemeStore`) are the exception to "no `set()` outside a listener" — they own their state directly and persist it via zustand's `persist` middleware with `createJSONStorage(() => AsyncStorage)`. Current store: `useThemeStore` — `mode: 'system' | 'light' | 'dark'`, `setMode(mode)` (also calls NativeWind's `colorScheme.set(mode)`, and `onRehydrateStorage` re-applies it on app launch so NativeWind and the persisted preference never drift).
